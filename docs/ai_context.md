...
- **Infrastructure:** Docker SDK for containerized tool hosts, Valkey for session/config persistence.
...
4. **WebSocket Hub (`internal/api/websocket`)**: Manages bidirectional communication with the frontend and session persistence via the `ValkeyStore` interface.
...
- **Testability:** Core services depend on interfaces (`LLMProvider`, `ToolProvider`, `ValkeyStore`) to allow mocking in unit and integration tests.
